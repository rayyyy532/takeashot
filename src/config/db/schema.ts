...
export const takeashotGeneration = table(
  'takeashot_generation',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
      .notNull()
    ...
    personAUrl: text('person_a_url'),
    personBUrl: text('person_b_url'),
  ...
  (table) => [
    index('idx_takeashot_user_id').on(table.userId),
    index('idx_takeashot_status').on(table.status),
  ]
);
